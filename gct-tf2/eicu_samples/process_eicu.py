...
def build_seqex(enc_dict,
  ...
  for _, enc in enc_dict.items():
    ...
    seqex.context.feature['patientId'].bytes_list.value.append(str(enc.patient_id +
                                                                       ':' + enc.encounter_id).encode())
    if enc.expired:
    ...
    dx_ids_bytes = [dx.encode() for dx in set(enc.dx_ids)]
    ...
    proc_ids_bytes = [proc.encode() for proc in set(enc.treatments)]
