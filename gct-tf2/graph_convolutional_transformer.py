...
class FeatureEmbedder(object):
    ...
    def __init__(self, vocab_sizes, feature_keys, embedding_size):
        ...
        self._params['visit'] = tf.compat.v1.get_variable(
...
class GraphConvolutionalTransformer(tf.keras.layers.Layer):
    ...
    def call(self, features, masks, guide=None, prior_guide=None, training=None):
        ...
        # during the process.
        mask_idx = tf.cast(tf.compat.v1.where(tf.equal(masks[:, :, 0], 0.)), tf.int32)
        ...
        if self._use_inf_mask:
            guide_idx = tf.cast(tf.compat.v1.where(tf.equal(guide, 0.)), tf.int32)
        ...
        if self._directed_attention:
            ...
            inf_matrix = tf.linalg.set_diag(inf_matrix, tf.zeros(num_codes))
            directed_mask = tf.linalg.band_part(inf_matrix, -1, 0)[None, None, :, :]
        ...
        for i in range(self._num_stack):
            ...
            post_attention = post_attention + features
            ...
            post_ffn = post_ffn + post_attention
...
def create_matrix_vdp(features, mask, use_prior, use_inf_mask, max_num_codes,
    ...
    Args:
      ...
      prior_scalar: A float value between 0.0 and 1.0 to be used to hard-code the
        diagonal elements of the prior matrix.

    Returns:
      guide: The guide matrix.
      prior_guide: The conditional probability matrix.
    """
    dx_ids = features['dx_ints']
    proc_ids = features['proc_ints']
...
class EHRTransformer(object):
    ...
    def get_prediction(self, model, feature_embedder, features, training=False):
        ...
        logits = tf.compat.v1.layers.dense(pre_logit, self._num_classes, activation=None)
    ...
    def model_fn(self, features, labels, mode):
        ...
        if mode == tf.estimator.ModeKeys.EVAL:
            # Define the metrics:
            metrics_dict = {
                'AUC-PR': tf.compat.v1.metrics.auc(labels, probs, curve='PR',
                                                   summation_method='careful_interpolation'),
                'AUC-ROC': tf.compat.v1.metrics.auc(labels, probs, curve='ROC',
                                                    summation_method='careful_interpolation')
            }
