...
class FeatureEmbedder(nn.Module):
    def __init__(self, feature_keys, vocab_sizes, hidden_size, hidden_dropout_prob):
        super().__init__()
        self.embeddings = {}
        self.feature_keys = feature_keys
        self.dx_embeddings = nn.Embedding(vocab_sizes['dx_ints'] + 1, hidden_size,
                                          padding_idx=vocab_sizes['dx_ints'])
        self.proc_embeddings = nn.Embedding(vocab_sizes['proc_ints'] + 1, hidden_size,
                                            padding_idx=vocab_sizes['proc_ints'])
        self.visit_embeddings = nn.Embedding(1, hidden_size)
...
class SelfAttention(nn.Module):
    def __init__(self, num_heads, hidden_size, stack_idx):
        super().__init__()
...
class Attention(nn.Module):
    def __init__(self, num_heads, hidden_size, hidden_dropout_prob, stack_idx):
        super().__init__()
        self.self_attention = SelfAttention(num_heads, hidden_size, stack_idx)
...
class GCTLayer(nn.Module):
    def __init__(self, num_heads, hidden_size, hidden_dropout_prob, stack_idx):
        super().__init__()
        self.attention = Attention(num_heads, hidden_size, hidden_dropout_prob, stack_idx)
...
class GCT(BaseModel):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [GCTLayer(self.num_heads, self.hidden_size, self.hidden_dropout_prob, i) for i in range(self.num_stacks)])
        self.embeddings = FeatureEmbedder(self.feature_keys, self.vocab_sizes, self.hidden_size,
