...
def visualize_attention(args, data, dx_map, proc_map, img_dir, attention_maps):
    ...
    for i in range(batch_size):  # for compact visualization
        ...
        if dx_ints_actual.shape[0] + proc_ints_actual.shape[
            0] <= 12:  # to fit the figsize, only 12 under actual codes can be annotated
    ...
    if len(viz_batch_idxs) == 0:
        print(
            "Cannot visualize attention map because there are so many actual codes. Please try another max_steps value.")
        return

    fig, axs = plt.subplots(ncols=len(viz_batch_idxs), nrows=1,
                            figsize=(12 * len(viz_batch_idxs), 12 * len(viz_batch_idxs)),
                            layout="constrained")  # need to increase fig size properly
    ...
    for i, v in enumerate(viz_batch_idxs):
        ...
        labels = ['visit', ] + ['D_' + rv_dx_map[v.item()] for v in dx_ints_actual] + ['T_' + rv_proc_map[v.item()] for
                                                                                       v in proc_ints_actual]
...
def main():
    ...
    if args.do_train:
        ...
        # attn_probs = outputs[-1][-1]
        # # the last element in output tuple, if you specify num_of_stacks > 1 then this would return the last stack attention
        # head_idx = 0
        # # original paper used one-head attention. If you specified num_of_heads more than 1, then you can change this value.
        # attention_map = attn_probs[:, head_idx, :, :].cpu().numpy()
        # # [batch_idx, head_idx, input_token_idx, output_token_idx]
        #
        # # Read map pickle files
        # import pickle
        # fold_path = os.path.join(args.data_dir, 'fold_{}'.format(args.fold))
